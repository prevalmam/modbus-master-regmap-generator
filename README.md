...
生成コード側では、各レジスタブロックごとに **read 系 (`modbus_sender_req_*`)** と **write 系 (`modbus_sender_set_*`)** の 2 本立て API が自動生成されます。`MODBUS_SLAVE_ADDR` は `modbus_reg_idx_master.h` で定義されているので、スレーブ側のアドレスに合わせて書き換えてください。
...
read/write どちらの API も送信が終われば処理は完結しており、以降は 4.4 で示す `modbus_reply_handler_master()` で応答を待つだけです。
...
#### 4.5.1 値の読み出し（getter）
    ...
    uint16_t mode = modbus_reg_get_device_mode();
    float pressure = modbus_reg_get_pressure();

#### 4.5.2 値の書き換え（setter）

    modbus_reg_set_target_pressure(450);
...
#### 4.6.1 単体値の例
    ...
    if (modbus_reg_edge_rising_device_status()) {
        // 0 → 1 に変化したときだけ実行
    }
...
#### 4.6.2 配列レジスタの例
    ...
    for (int i = 0; i < NUM_SENSORS; i++) {
        if (modbus_reg_edge_changed_sensor_values(i)) {
            // sensor_values[i] が変化
        }
    }
